...
company_columns = [col for col in columns if "회사명" in col]
...
for _, row in df.iterrows():
    ...
    if is_empty:
        ...
        temp_data.sort(key=lambda x: x.get("근무기간(시작)", pd.NaT) or pd.Timestamp.max)
...
final_columns = ["회원번호", "경력연번"] + set_columns_order
